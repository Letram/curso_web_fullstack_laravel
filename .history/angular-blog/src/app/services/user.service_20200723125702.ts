...
export class UserService {
  ...
  public update(user: { id: any; name: string; email: string; description: string; }): Observable<any> {
    let headers = new HttpHeaders().set("Content-type", "application/json");
    headers.append("Authorization", this.token);
    return this._http.put(global.url_api+`/users/${this.identifiedUser.sub}`, user, {headers:headers});
  }
